...
setMethod(
    ...
    definition = function(cTobject, subsets, ribosomal,
        ...
        gcmax[gcmax == 0] <- 0.5 # account for codons not used in ref. set
        x <- as.integer(droplevels(aa))
        gcm <- sapply(colnames(gc), function(y) # freq of the most freq. codons
            gc[, y] <- gcmax[x, y])
...
setMethod(
    ...
    definition = function(cTobject, subsets, ribosomal,
        ...
        sapply(colnames(gcm), function(y){
            ...
            cnt[which(t(ra) < 0.9, arr.ind = TRUE)] <- NA # as impl. in INCA
...
setGeneric(
    name = "GCB",
    def = function(cTobject, seed = logical(), ribosomal = FALSE, perc = 0.05,
                   id_or_name2 = "1", alt.init = TRUE, stop.rm = FALSE,
                   filtering = "none", len.threshold = 80) {
        standardGeneric("GCB")
    }
...
setMethod(
    ...
    definition = function(cTobject, seed, ribosomal, perc,
        ...
        if (length(seed) == 0)
            ...
            seed <- list(seed=seed)
        gc <- expectedCU(cTobject, gCobject, seed, self = FALSE, ribosomal)
        gcb_prev <- numeric(nrow(counts))
        iter <- 0
        repeat {
            cb <- log(gc / colMeans(fc, na.rm = TRUE))
            cb[gc == 0] <- -5
            gcb <- rowSums(t(t(counts) * as.numeric(cb)), na.rm = TRUE) / len
