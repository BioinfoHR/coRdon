...
calcMilc<-function(s, subsets=list(), self = TRUE) {
  ...
  # add a dummy self selection to subsets if self == TRUE
  if(self == TRUE) {
  ...
  o<-s[,nostops]
  for(i in nostops) o[[i]] <- as.integer(o[[i]])
  o<-as.matrix(o)
  l <- s$len
  ...
  gc_list <- sapply(subsets, function(y) {
    if(is.vector(y, mode = "logical")) {
      sel_sum <- colSums(o[y,]) # this should never give error, beecause we tested for equal length
    } else {
      sel_sum <- colSums(as.matrix(y[,nostops]))
    }
#    sel_sum <- colSums(sel) # add counts per codon
  ...
  corr1 <- t(t(byrci(o)) * as.vector(acnt-1))
