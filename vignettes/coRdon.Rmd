...
For each sequence, in each sample, we can calculate codon usage (CU) bias. In order to do this, we need to count occurrences of each codon in each sequence. This can be done by storing sequences in each sample as a `codonTable` object. If we had the sequences in .fasta format, we could use `readSet()` to read .fasta files and store sequences as a `DNAStringSet` object, which could then be converted to `codonTable` using the constructor method `codonTable()`.  
...
KEGGREST package can be used to match pathway identifiers to pathway descriptions.
...
Thus created matrix, with genes in rows and samples in columns, can be directly used as an argument to `Heatmap()` function for plotting. However, for the purpose of neat visualization, we can again make use of the KEGGREST package to match pathway identifiers to pathway descriptions, and we can also remove non-informative categories from the matrix.
...
head(mat)
rows <- apply(mat, 1, function(x) abs(diff(x)) < 10)
mat <- mat[!rows,]
