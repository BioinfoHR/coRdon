...
We can plot enriched and depleted KO categories on an MA plot, with significant enrichment or depletion defined as those having the p-value below the level of significance `siglev` 0.05 (these are default parameters, one can also use adjusted p-value, by setting `pvalue` = "padj", and different values of `siglev`).  
