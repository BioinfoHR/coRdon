...
* functional enrichment analysis for the subset of genes that have CU similar to a reference set of highly expressed genes, and are therfore predicted to be highly expressed themselves.  

### Loading sequences

Sequences from human gut microbiome samples of healthy individuals and liver chirosis patients ([Quin et al. 2014](https://go.nature.com/2G7QdpC)) were processed, assembeled and used to predict ORFs, which were then annotated with a KO (KEGG orthology) function ([Fabijanic and Vlahovicek 2016](https://bit.ly/2DRfiz6)).  

For each sequence, in each sample, we can calculate codon usage (CU) bias. In order to do this, we need to count occurences of each codon in each sequence. This can be done by storing sequences in each sample as a `codonTable` object. If we had the sequences in .fasta format, we could use `readSet()` to read .fasta files and store sequences as a `DNAStringSet` object, which could then be converted to `codonTable` using the constructor method `codonTable()`.  

For the purpose of this vignette, however, we will load two `codonTable` objects from `coRdon` package, HD59 and LD94, containing codon counts for metagenomic gut sample from healthy individual, and from liver chirosis patient, respectively.
...
`codonTable` object stores codon counts for each sequence, as well as some additional metadata, namely sequence ID infered from a .fasta file or DNAStringSet object, sequence length in codons and KO / COG annotation. Note that not all of these need be present in every `codonTable` object - only the codon counts, and inferred lengths are mandatory. They can be accesed using `codonCounts()` and `getlen()` (not to be confused with `length()` which returns number of sequences in a `codonTable` object):

```{r}
head(codonCounts(HD59), 3)
getlen(HD59)
length(HD59)
```

We can get get IDs, KO or COG annotations in a similar way, if these are present:
...
Now we can calculate CU bias for every sequence. There are many stasitics that measure codon usage, and several widely used ones are implemented in coRdon. For example, Measure Independent of Length and Composition (MILC) can be calculated for sequences in the `codonTable` like so:
...
Another important consideration is that MILC, like most of the other CU bias statistics (and contrary to its name), is dependent on the length of sequence for which it is calculated. In order to obtain meaningful values, it is recommended that sequences should be least 80 codons long. To that end, either soft or hard filtering step can be included in calculation of CU statistics. Hard filtering will remove all of the sequences from `codonTable` that are shorter than some specified threshold (length in codons), whereas soft filtering will produce a message if there are such sequences in the set, but will not remove them.
...
Now we calculate MILC values with respect to overall sample CU and ribosomal genes CU, removing beforehand sequences shorter than 80 codons (default value, use `len.threshold` to change this):

```{r}
milc <- MILC(HD59, ribosomal = TRUE, filtering = "hard")
```

Other CU statistics can be calculated in the same way as `MILC()`, using one of the functions: `B()`, `MCB()`, `ENCprime()`, `ENC()` or `SCUO()`. Note however, that when calculating ENC and SCUO, one need not provide a subset of reference genes, because these statistics measure distance in codon usage to uniform usage of synonymous codons.  
...
bplot(x = "ribosomal", y = "self", data = milc) +
    labs(x = "MILC distance from ribosomal genes",
         y = "MILC distance from sample centroid")
```

The argument `annotations` need to be specified when we wish to indicate certain genes on the plot. For example, to indicate ribosomal genes, provide character vector of annotations corresponding to values in `x`, `y` and `data`, and set `ribosomal = TRUE`. Note that, since we filtered sequences when calculating MILC, we now need to provide the annotations for those sequences only.
...
To indicate any other gene(s), use `reference` argument. `reference` takes logical vector in which `TRUE` corresponds (by position) to genes that are to be indicated on the plot, and `FALSE` to all the othe genes, or character vector of annotations for the genes to be indicated on the plot.
...
### Enrichment of functions within highly expressed genes in metagenomes.
...
We can plot enriched and depleted KO categories on a MA plot, with significant enrichment or depletion defined as those having the p value, `pvals`, below the level of significance `siglev` 0.05 (these are also default parameters).  

```{r}
enrichMAplot(enr, pvalue = "pvals", siglev = 0.05)
```

In order to determine functions that are enriched among highly expressed genes, we need to map KO annotations to a broader onthology. This can be done using `reduceCrossTab()` method which can take a `crossTab` object as its first argument, and map KO categories to either KEGG Pathway, KEGG Module, or COG functional categories, depending on the value of `target` argument.  
...
Another way to visualize the enrichment would be by plotting the results on a barplot. Here bars are coloured by level of significance, `padj`, whereas their height is the relative enrichment `"enrich"` (can also be mean average of scaled counts, `"A"`, or scaled counts ratio, `"M"`). If we specify `siglev`, only those categories for which associated `pvalue` ("pvals" or "padj") is below the given value will be ploted.  
...
For example, if we performed CU-based enrichment analysis on the two metagenomic samples described at the beginning of this vignette, and wanted to see how the results compare by plotting them on a heatmap (e.g. using the amazing ComplexHeatmap package), we could do the following:
